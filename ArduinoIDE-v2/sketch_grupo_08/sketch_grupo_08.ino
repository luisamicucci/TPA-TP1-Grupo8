...
void loop() {
  ...
  if (lectura == LOW) {                 // Pulsador presionado (lógica invertida por pull-up)
    contador += 1;                      // Incrementar contador de modo
    if(contador > 4) {                  // Si supera modo 4, volver al modo 1
      contador = 1;
    }
...
void humedadMinima() {
  ...
  if (touchRead(TOUCH_PLUS) < 30) {             // Umbral de detección capacitiva
    humedadMinDeseada += 1;
